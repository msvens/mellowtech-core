...
public class RecordFileBuilder {
  ...
  public RecordFile build(String fileName) throws IOException{
    RecordFile rf = create(strategy, maxBlocks, fileName, blockSize, reserve, splitMaxBlocks, splitBlockSize);
    if(spanned){
      if(iterate){
  ...
  private static RecordFile create(Strategy s, Integer maxBlocks, String fName, 
      int blockSize, int reserve, int splitMaxBlocks, int splitBlockSize) throws IOException {
    switch(s) {
      case DISC:
