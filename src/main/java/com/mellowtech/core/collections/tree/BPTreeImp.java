...
import com.mellowtech.core.io.BlockFile;
 ...
 * @version 1.0
 */
public class BPTreeImp<K extends ByteComparable, V extends ByteStorable>
implements BTree <K, V> {
  ...
  protected BPlusHelper <K,V> helper;
  protected K keyType;
   ...
   */
  public BPTreeImp(String fName, K keyType, V valueType) throws Exception {
    // open the file:
    this.fName = fName;
    indexName = fName + ".idx";
    valueName = fName + ".val";
   ...
   * @param valueBlockSize Block size in key/value file. The size has to be at least twize
   ...
   */
  public BPTreeImp(String fName, K keyType,
                   V valueType, int valueBlockSize, int indexBlockSize)
          throws IOException {
  ...
  private void openIndex(boolean newIndex, int blockSize) throws IOException {
    if (newIndex) {
      indexFile = new BlockFile(indexName, blockSize, 1024, 1024);
      indexFile.clear();
    } else {
  ...
  private void openValues(boolean newIndex, int blockSize) throws IOException {
    if (newIndex) {
      valueFile = new BlockFile(valueName, blockSize, 1024 * 1024, 0);
      valueFile.clear();
    } else {
