 ...
 * @version 1.0
 */
public abstract class ByteStorable <T> implements Comparable <T> {

  protected boolean doNew = true;

  public ByteStorable deepCopy(){
   ...
   *              if an error occurs
   */
  public int compareTo(T t) throws UnsupportedOperationException {
  ...
  public ByteStorable <T> separate(ByteStorable <T> first, ByteStorable <T> second) {
    ByteStorable <T> toRet = first.compareTo((T) second) > 0 ? first : second;
    // now copy:
