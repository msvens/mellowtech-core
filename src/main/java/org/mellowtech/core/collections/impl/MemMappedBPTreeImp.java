...
public class MemMappedBPTreeImp<A,B extends BComparable<A,B>,C,D extends BStorable<C,D>>
   ...
   * @param inMemory       if true the entire btree (including key/values) are kept in a memory mapped file
   * @param maxBlocks      max number of key/value blocks in this btree
  ...
  protected void handleMerge(BCBlock <BTreeKey.Entry<B>, BTreeKey <B>> sb, BPlusReturn <B,D> ret, int cBlock,
    ...
    BCBlock <BTreeKey.Entry<B>, BTreeKey <B>> sib;
    // redistribute:
  ...
  private void insertSeparator(BTreeKey <B> sep, SBBNo <B> [] levels, int current,
    ...
    if (levels[current] == null) { // we have to create a new level
      ...
      levels[current].sb = helper.newIBlock();
  ...
  protected BPlusReturn <B,D> insertKeyValue(KeyValue<B,D> keyValue, int bNo,
                                       boolean update) throws IOException {
    BCBlock <KeyValue.KV<B,D>, KeyValue <B,D>> sb, sb1;
    try {
  ...
  protected void createRoot(BTreeKey <B> rootKey) throws IOException {
    ...
    BCBlock<BTreeKey.Entry<B>, BTreeKey<B>> sb = helper.newIBlock();
  ...
  private void openIndex(boolean newIndex, int indexBlockSize,
                         int valueBlockSize, boolean inMemory, int maxBlocks, int maxIndexBlocks) throws IOException {

    RecordFileBuilder sfb = new RecordFileBuilder();
    //maxBlocks = Math.max(DEFAULT_MAX_BLOCKS, maxBlocks);
    sfb = inMemory ? sfb.memSplit() : sfb.split();
    sfb.blockSize(valueBlockSize).splitBlockSize(indexBlockSize).maxBlocks(maxBlocks).reserve(1024).splitMaxBlocks(maxIndexBlocks);
    if(newIndex){
      splitFile = (SplitRecordFile) sfb.build(indexName);
      splitFile.clear();
    }
    else {
      sfb.maxBlocks(null);
