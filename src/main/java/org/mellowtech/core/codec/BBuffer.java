 ...
 */
public class BBuffer<A extends Comparable<? super A>> implements RangeIterable<A,A> {
   ...
   */
  public BBuffer(ByteBuffer block, BCodec<A> codec) {
    this(block, codec, false, null, (short) -1);
  }
   ...
   */
  public BBuffer(int blockSize, BCodec<A> codec, PtrType ptrType, short reservedSpace) {
    this(ByteBuffer.allocate(blockSize), codec, true, ptrType, reservedSpace);
  }
   ...
   */
  public BBuffer(int blockSize, BCodec<A> codec, PtrType ptrType) {
    this(ByteBuffer.allocate(blockSize), codec, true, ptrType, (short) 0);
  }
   ...
   */
  public BBuffer(ByteBuffer block, BCodec<A> codec, PtrType ptrType, short reservedSpace) {
    this(block, codec, true, ptrType, reservedSpace);
  }
   ...
   * @param ptrType ptr type
   */
  public BBuffer(ByteBuffer block, BCodec<A> codec, PtrType ptrType){
    this(block, codec, ptrType, (short) 0);
  }
   ...
   */
  protected BBuffer(ByteBuffer block, BCodec<A> codec, boolean newBlock,
  ...
  private static <A extends Comparable <? super A>> void getKeysPrevious(BBuffer<A>[] blocks, int index,
                                          int optimal) {
    int blockBytes;
    A current;
    int diff;
    ...
    while (true) {
      ...
      diffPut = Math.abs(blockBytes + blocks[index].codec.byteSize(current) - optimal);
  ...
  private static <A extends Comparable <? super A>> void putKeysPrevious(BBuffer<A>[] blocks, int index,
                                          int optimal) {
    int blockBytes;
    A current;
    int diff;
   ...
   * @param <A> value type
   */
  public static <A extends Comparable<? super A>> void redistribute(BBuffer<A>[] blocks) {
  ...
  public A delete(int idx) {
    ...
    block.position(pPos);
    A toDelete = codec.from(block);
    int firstPos = capacity - bytesWritten;
    int byteSize = codec.byteSize(toDelete);
   ...
   */
  public boolean fits(A element) {
    return reservedSpace + headerSize + bytesWritten + ((high + 1) * ptrSize)
        + codec.byteSize(element) <= capacity;
  }
   ...
   */
  public boolean fits(BBuffer<A> other, A additional) {
    int totDataBytes = other.getDataBytes() + getDataBytes()
        + codec.byteSize(additional);
  ...
  public int insert(A element) {
    ...
    int pPos = capacity - bytesWritten - codec.byteSize(element);
    ...
    high++;
    writeNumElements(high);
    bytesWritten += codec.byteSize(element);
    writeBytesWritten(bytesWritten);
    block.position(pPos);
    codec.to(element, block);
    return pos;
  }
   ...
   */
  public boolean insertUnsorted(A element) {
    if (!fits(element))
      return false;
    int pPos = capacity - bytesWritten - codec.byteSize(element);
  ...
  public Iterator<A> iterator(boolean descend, A from, boolean fromInclusive,
    ...
    return descend ? new BBufferDescendIter(this, from, fromInclusive, to, toInclusive) :
        new BBufferIter(this, from, fromInclusive, to, toInclusive);
  }
   ...
   * @return this buffer
   */
  public BBuffer<A> merge(BBuffer<A> other) {
   ...
   * @see Arrays#binarySearch(Object[], Object)
   */
  public int searchBC(A element) {
    int low = 0;
    int highSearch = high - 1, mid;
    ByteBuffer bbKey = codec.to(element);
    while (low <= highSearch) {
      mid = (low + highSearch) / 2;
      int midValOffset = getPhysicalPos(mid);
      int cmp = codec.byteCompare(midValOffset, block, 0, bbKey);
      if (cmp < 0)
        low = mid + 1;
      else if (cmp > 0)
        highSearch = mid - 1;
   ...
   * @return this buffer
   */
  public BBuffer<A> sort(boolean parallelSort) {
    @SuppressWarnings("unchecked")
    A toSort[] = (A[]) new Comparable[high];
    int elems = high;
   ...
   */
  public BBuffer<A> split() {
    return split(new BBuffer<>(capacity, codec, ptrType, (short) (reservedSpace - 2)));
  }
   ...
   * @return buffer with larger elements
   */
  public BBuffer<A> split(BBuffer<A> other) {
    ...
    while (numWritten < half) {
      ...
      numWritten += codec.byteSize(lastKey);
    }
   ...
   */
  public void update(A element, int idx) {
    block.position(getPhysicalPos(idx));
    codec.to(element, block);
  }
  ...
  int getLowerBound(A key, boolean inclusive) {
    int pos = 0;
  ...
  int getUpperBound(A key, boolean inclusive) {
    int pos = high - 1;
