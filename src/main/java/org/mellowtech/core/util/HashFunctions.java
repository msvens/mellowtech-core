...
public class HashFunctions {
  ...
  static int tab[] = new int[] { 0x00000000, 0x77073096, 0xee0e612c,
   ...
   * 
   * This hash function is based on the good old CRC-32 (Cyclic Redundancy Check
   * with 32 Bit) algorithm as invented by Mark Adler. It is one of the hash
   * functions with medium performance but with very good distribution. So it
   * can be considered as a rock solid general purpose hash function. It should
   * be used if good distribution is more important than high performance.
   */
  public static int hash_macrc32(byte[] key) {
    int hash;
    hash = 0xffffffff;
    int len = key.length;
    int i = 0;
    while (len-- > 0)
      hash = tab[(int) ((hash ^ key[i++]) & 0xff)] ^ (hash >>> 8);
    hash ^= 0xffffffff;
    return hash;
  }

  public static int hash_djbx33(byte[] key) {
    int hash = 5381;
    int len = key.length;
    ...
    switch (len) {
    case 7:
      hash = ((hash << 5) + hash) + key[i++]; /* fallthrough... */
    case 6:
      hash = ((hash << 5) + hash) + key[i++]; /* fallthrough... */
    case 5:
      hash = ((hash << 5) + hash) + key[i++]; /* fallthrough... */
  ...
  private static void mix(Mixer m) {
    m.a -= m.b;
    m.a -= m.c;
    m.a ^= (m.c >>> 13);
    m.b -= m.c;
  ...
  public static int hash_bjddj(byte[] k) {
    Mixer m = new Mixer();
    int length = k.length;
    int len = length;

    m.a = m.b = 0x9e3779b9; /* the golden ratio; an arbitrary value */
    m.c = 0;

    /* handle most of the key */
    int i = 0;
    while (len >= 12) {
      m.a += (k[i + 0] + ((int) k[i + 1] << 8) + ((int) k[i + 2] << 16) + ((int) k[i + 3] << 24));
      m.b += (k[i + 4] + ((int) k[i + 5] << 8) + ((int) k[i + 6] << 16) + ((int) k[i + 7] << 24));
      m.c += (k[i + 8] + ((int) k[i + 9] << 8) + ((int) k[i + 10] << 16) + ((int) k[i + 11] << 24));
      mix(m);
      i += 12;
      len -= 12;
    }
